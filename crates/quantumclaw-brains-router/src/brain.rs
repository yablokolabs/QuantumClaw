...
//! [`quantumclaw_core::SolverRegistry`] for a backend by name and treats every
//! backend identically.

use crate::compiler::{assignment_problem, AssignmentWeights};
use crate::constraints::{solution_violations, RouterViolation};
use crate::decoder::{
...
use crate::kpis::{self, RouterKpis};
...
impl QRouterBrain {
            ...
            Some(name) => {
                ...
                let summary = Decomposition {
                    ...
                    subproblems: subproblems
                        .iter()
                        .map(|piece| quantumclaw_brains::SubproblemSummary {
                            id: piece.id.clone(),
    ...
    async fn solve_subproblem(
        ...
        let decision = match explicit.as_deref() {
            Some(BACKEND_CLASSICAL) => crate::routing_policy::RoutingDecision {
                backend: None,
                reason: "the caller asked for the classical path explicitly".into(),
            },
            Some(name) => crate::routing_policy::RoutingDecision {
    ...
    pub fn ledger_records(reports: &[SubproblemReport]) -> Vec<LedgerRecord> {
        reports
            ...
            .map(|report| LedgerRecord {
                ...
                size_bucket: crate::routing_policy::size_bucket(report.variables),
...
fn seeded_problem(mut problem: DecisionProblem, seed: Option<u64>) -> DecisionProblem {
    if let Some(seed) = seed {
        problem.metadata.data.insert(
            quantumclaw_core::hints::SAMPLER_SEED.into(),
            seed.to_string(),
        );
    }
    problem
